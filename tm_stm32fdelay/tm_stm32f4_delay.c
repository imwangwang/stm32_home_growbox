...
void TM_DELAY_Init(void) {
	...
	if (SysTick_Config(SystemFrequency_SysClk / 1000)) {
		/* Capture error */
		while (1);
	}
		...
		mult = SystemFrequency_SysClk / 7000000;
	#else
		/* Set multiplier for delay under 1us with pooling mode = not so accurate */
		mult = SystemFrequency_SysClk / 3000000;
	#endif
#endif
...
void TM_DELAY_INT_InitTIM(void) {
	...
	TIM_TimeBaseStruct.TIM_Prescaler = SystemFrequency_SysClk / (1000000 * (SystemFrequency_SysClk / TIM_Data.TimerFrequency)) - 1; /* With prescaler for 1 microsecond tick */
	...
	TIM_OC1Init(TM_DELAY_TIM, &TIM_OCInitStructure);


	/* Clear DelayTIM update pending flag */
	TIM_ClearFlag(TM_DELAY_TIM, TIM_FLAG_Update);
