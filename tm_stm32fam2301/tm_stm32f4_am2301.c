...
TM_AM2301_t TM_AM2301_INT_InitPins(void) {
	/* Enable clock */
	RCC_APB2PeriphClockCmd(AM2301_RCC, ENABLE);
	/* Set options */
	AM2301_GPIO_InitStruct.GPIO_Pin = AM2301_PIN;
	AM2301_GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_PP;
...
TM_AM2301_t TM_AM2301_INT_Read(TM_AM2301_Data_t* data) {
	...
	uint8_t i, j, d[5];
	uint8_t start_signal = 18;
	
	/* Pin output */
	AM2301_PIN_OUT;
	/* Set pin low at least for 18ms */
	AM2301_PIN_LOW;
	...
	if (d[2] & 0x80) {
		data->Temp = -((d[2] & 0x7F) * 10 + d[3]);
	} else {
		data->Temp = (d[2]) * 10 + d[3];
	}
